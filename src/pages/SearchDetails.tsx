...
export default function SearchDetails() {
  const location = useLocation(); // ambil yang tertulis di query params url
  ...
  useEffect(() => {
    const query = new URLSearchParams(location.search).get("query"); // ambil yg berada di dalam query
    if (query) {
      setSearchQuery(query);
      performSearch(query); // lempar hasil paramsnya
    }
  }, [location.search]);
  ...
  // handle change untuk mengirimkan query yg di input user di form search
